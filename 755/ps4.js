...
function toggle_payload(pld){
	if(pld == "exploit"){
		document.getElementById("progress").innerHTML="执行越狱中...请稍后!!";
		preloadScripts(['jb.js']);
	}else if(pld == "exploit_old"){
		document.getElementById("progress").innerHTML="执行越狱中...请稍后!!";
		preloadScripts(['oldjb.js']);
	}else if(pld == "binloader"){
		document.getElementById("progress").innerHTML="等待载荷注入.. 端口9021..";
		preloadScripts(['preloader.js', 'loader.js']);
	}else if(pld == "mira75X"){
		document.getElementById("progress").innerHTML="加载MIRA破解.. 请稍后..";
		if(fw=="755"){
		...
		document.getElementById("progress").innerHTML="加载mira+伪装FW9.99.. 请稍后..";
		preloadScripts(['preloader.js', 'mira2b.js', 'loader.js']);
	}else if(pld == "goldhen"){
		document.getElementById("progress").innerHTML="加载GoldHenv1.1..请稍后..";
		if(fw=="755"){
		...
		document.getElementById("progress").innerHTML="加载GoldHenv1.1b3.. 请稍后..";
		if(fw=="755"){
		...
		document.getElementById("progress").innerHTML="加载GoldHenv1.0.. 请稍后..";
		if(fw=="755"){
		...
		setTimeout(function(){document.getElementById("progress").innerHTML="FTP服务.. 端口1337.."; }, 7000);
		preloadScripts(['preloader.js', 'ftp.js', 'loader.js']);
	}else if(pld == "app2usb"){
		document.getElementById("progress").innerHTML="正在加载有效载荷..请稍候..";
		...
		document.getElementById("progress").innerHTML="正在加载有效载荷..请稍候..";
		preloadScripts(['preloader.js', 'fan'+tempC.value+'.js', 'loader.js']);
...
function payload_finished(payload)
{
	if(payload == "binloader"){
		setTimeout(function(){document.getElementById("progress").innerHTML="等待有效载荷！ 请将有效载荷发送到端口9021"; }, 7000);
	} else if(payload != "exploit" && payload != "exploit_old"){
		setTimeout(function(){document.getElementById("progress").innerHTML="有效负载已成功加载!!"; }, 7000);
	}
}
...
function leakJSC() {
	...
	debug_log("-> JS数组缓冲区视图: " + g_jsview_leak);
...
function dumpTargetObj() {
	debug_log("-> 计时器: " + g_timer_leak);
	debug_log("-> 标题信息: " + g_message_heading_leak);
	debug_log("-> 正文信息: " + g_message_body_leak);
}
...
function sprayHTMLTextArea() {
	debug_log("开始执行webkit利用...(超20次内存不足时，请关闭页面，重启浏览器再试)");
