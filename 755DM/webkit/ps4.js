...
function setupRW() {
	...
	if (g_ab_slave === null)
		die("[!] 失败，找不到从属JS数组缓冲区视图（请重启浏览器再试）");
	...
	if(!read64(g_jsview_butterfly.sub(16)).equals(new Int64("0xffff000000001337")))
		die("[!] 失败，无法设置添加rof / 虚拟obj（请重启浏览器再试）");
	if(localStorage.autoExploit=="true")
		debug_log("WebKit漏洞利用完成。请继续选择合适的越狱方式 !!");
	else
		debug_log("WebKit漏洞利用完成。请继续选择合适的越狱方式 !!");
...
function toggle_payload(pld){
	if(pld == "exploit"){
		document.getElementById("progress").innerHTML="正在越狱..请稍候!!";
		preloadScripts(['jb/jb.js']);
	}else if(pld == "exploit_old"){
		document.getElementById("progress").innerHTML="正在越狱..请稍候!!";
		preloadScripts(['jb/oldjb.js']);
	}else if(pld == "binloader"){
		document.getElementById("progress").innerHTML="等待有效载荷..将有效载荷发送到端口9020..";
		preloadScripts(['payloads/preloader.js', 'payloads/loader.js']);
	}else if(pld == "mira75X"){
		document.getElementById("progress").innerHTML="载入MIRA破解 ..请稍候..";
		if(fw=="755"){
		...
		document.getElementById("progress").innerHTML="载入MIRA破解 + 伪装 ..请等待..";
		preloadScripts(['payloads/preloader.js', 'payloads/mira2b.js', 'payloads/loader.js']);
	}else if(pld == "ftp"){
		setTimeout(function(){document.getElementById("progress").innerHTML="FTP已加载。访问端口1337。"; }, 7000);
...
function payload_finished(payload)
{
	if(payload == "binloader"){
		setTimeout(function(){document.getElementById("progress").innerHTML="等待有效载荷！ 将有效载荷发送到端口9021"; }, 7000);
...
function sprayHTMLTextArea() {
	debug_log("->开始执行WEBkit漏洞利用(超20次内存不足时，请重启PS4再试)");
